...
exports.handler = async function(event) {
    ...
    try {
        ...
        const model = genAI.getGenerativeModel({ model: "gemini-pro", tools: [{ functionDeclarations: [tools.find_relevant_services] }] });

        // --- SOLUCIÓN: INSTRUCCIONES DENTRO DEL HISTORIAL ---
        // En lugar de 'systemInstruction', simulamos ser el 'usuario' dando las reglas.
        const instructions = {
            role: "user",
            parts: [{
                text: `
        ...
        const geminiHistory = history.map(turn => ({ role: turn.role, parts: [{ text: turn.content }] }));
        
        // El historial ahora empieza con nuestras instrucciones, seguido de la conversación real.
        ...
        if (functionCalls && functionCalls.length > 0) {
            console.log("IA solicita llamar a la herramienta:", functionCalls[0].name);
